...
# Truy vấn dữ liệu từ bảng mst_flash_230321878_data
print("Đang thực hiện truy vấn dữ liệu...")
cur.execute("""
    SELECT
    flash_policy."warehouse_name" AS "Sender",
    msx.*,
    flash_policy.email
    FROM mst_data.mst_flash_230321878_data msx
...
# Lọc chỉ những cột cần thiết
df = df[['Sender', 'Waybill Number', 'Sender Phone', 'Receiver Name', 'Receiver Phone',
         'Weight (gram)', 'Shipping Fee', 'Shipping Fee Tax', 'COD', 'COD Fee',
         'COD Fee Tax', 'Insurance Value', 'Insurance Fee', 'Return Fee',
         'Return Fee Tax', 'Return Fee Adjustment', 'Total Fee', 'email']]

# Nhóm dữ liệu theo "Sender Name"
grouped = df.groupby("Sender")
print("Đã nhóm dữ liệu theo Sender.")

# Đường dẫn lưu file Excel tạm
...
def send_email(recipient_email, subject, body, attachment_path):
    try:
        ...
        print(f"Email đã gửi đến {recipient_email} với tệp đính kèm {os.path.basename(attachment_path)}")
    except Exception as e:
...
for sender_name, group in grouped:
    ...
    today_date = date.today().strftime("%Y%m%d")  # Lấy ngày hôm nay theo định dạng YYYY-MM-DD
    subject = f"{today_date}-CA63-FLASH-{sender_name}_SOA"
    print("Subject:", subject)  # Kiểm tra tiêu đề email
    ...
    print(f"Tính toán xong các tổng phí cho {sender_name}.")
    ...
    # Tạo tên file Excel
    file_name = f"{today_date}-CA63-FLASH-{sender_name.replace(' ', '_').replace('(', '').replace(')', '')}_SOA.xlsx"
    file_path = os.path.join(output_dir, file_name)

    print(f"Đang tạo file Excel: {file_path}")

    # Xuất dữ liệu nhóm ra file Excel
    group.drop(columns=["email", "Sender Name"], inplace=True)  # Không lưu email và Sender Name trong Excel

    # Tạo và ghi dữ liệu vào hai sheet trong Excel
    with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
        # Lưu DataFrame vào sheet "Order Detail"
        group.to_excel(writer, sheet_name="Order Detail", index=False)

        # Tạo dữ liệu cho sheet "BILLING STATEMENT"
        billing_data = [
        ...
        # Chuyển dữ liệu thành DataFrame và lưu vào sheet "BILLING STATEMENT"
